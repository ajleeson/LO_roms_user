...
# elif defined SHOREFACE
      ...
      f0=1.0E-04_r8
      beta=0.0_r8
#elif defined IDEAL_WITHV
      f0=user(1)
      beta=0.0_r8
      depth=user(2)
      Xsize=user(6)*1000.0_r8
      Esize=Xsize
#elif defined COD_1DMIX
! 2 km square water column for 1-D mixing example
      Xsize=2000.0_r8
      Esize=2000.0_r8
      f0=user(1)
      beta=0.0_r8
      depth=user(2)
#elif defined COD_CTW
      Xsize=user(6)*1000.0_r8      ! length E-W periodic
      Esize=user(7)*1000.0_r8
      f0=user(1)
      beta=0.0_r8
      depth=3000.0_r8              ! maximum water depth
#elif defined COD_BIOSED_ESTUARY
      Xsize=user(10)*1000.0_r8
      Esize=user(11)
      f0=user(1)
      beta=0.0_r8
      depth=user(2)
#elif defined COD_UPWELL || defined COD_UPWELL_LC2004 || defined COD_RAP
! east-west channel for 2-layer upwelling example
      Xsize= 1.0_r8*1000.0_r8       ! 1-km in east-west periodic direction   
      Esize=user(2)*1000.0_r8       ! width in north-south y-direction
      depth=user(3)                 ! constant water depth
      f0=user(1)
      beta=0.0_r8
#else
...
!
     ...
     &           (1.0_r8+TANH((yr(i,j)-val1)/10000.0_r8))
      ...
      END DO
#elif defined COD_BIOSED_ESTUARY
! linearly decreasing depth in the XI-direction
      DO j=JstrT,JendT
        DO i=IstrT,IendT
! change this segment to alter the shape of the estuary bathymetry
! variable named depth was set above to be the value in user(2)
! variable named Xsize was set above to be the value in user(11)
          h(i,j)=depth-xr(i,j)/Xsize*(depth-user(12))
        END DO
      END DO
#elif defined COD_CTW
! exponentially increasing depth in the ETA-direction
! user(2) is lambda*L in the idealized CTW problem in lectures 
! user(3) is L shelf width in km  
! min depth is calculated here so lambdaL changes both width and steepness of shelf
      DO j=JstrT,JendT
        DO i=IstrT,IendT
          h(i,j)=MIN(depth,                                             &
                 depth*EXP(2.0_r8*user(2)/user(3)*yr(i,j)/1000.0_r8-    &
     &                     2.0_r8*user(2)))
        END DO
      END DO
#elif defined COD_UPWELL_LC2004 || defined COD_RAP
! linear slope (user(4)) to uniform depth region
! Lentz and Chapman (JPO 2004)
      DO j=JstrT,JendT
        DO i=IstrT,IendT
          h(i,j)=MIN(depth,5.0_r8+user(4)*yr(i,j))
        END DO
      END DO
#elif defined ESTUARY_TEST
      DO j=JstrT,JendT
     ...
     &           (1.0_r8+TANH((yr(i,j)-100000.0_r8)/20000.0_r8))
      ...
      DO j=JstrT,JendT
        ...
        DO i=MAX(6,IstrT),IendT
          h(i,j)=depth+REAL(Lm(ng)-i,r8)*(2.0_r8-depth)/               &
     &                 REAL(Lm(ng)-6,r8)
        END DO
      END DO
#elif defined RIVERPLUME2
