...
!!          t(i,j,k,1,itemp)=14.0_r8+8.0_r8*EXP(z_r(i,j,k)/50.0_r8)
        ...
        END DO
      END DO
# elif defined COD_1DMIX || defined COD_BIOSED_ESTUARY || defined COD_CTW || defined COD_UPWELL_LC2004 || defined COD_RAP || defined IDEAL_WITHV
      DO k=1,N(ng)
        DO j=JstrR,JendR
          DO i=IstrR,IendR
            t(i,j,k,1,itemp)=T0(ng)+user(5)*z_r(i,j,k)
#  ifdef SALINITY
#    ifdef COD_RAP
            IF (yr(i,j).ge.user(8)*1000.0_r8) THEN       
              t(i,j,k,1,isalt)=S0(ng)
            ELSE
              t(i,j,k,1,isalt)=user(9)
            END IF
#    else    
            t(i,j,k,1,isalt)=S0(ng)
#    endif       
#  endif
        ...
        END DO
      END DO
# elif defined COD_UPWELL
      DO k=1,N(ng)
        DO j=JstrR,JendR
          DO i=IstrR,IendR
            IF (z_r(i,j,k).ge.-user(4)) THEN
              t(i,j,k,1,itemp)=T0(ng)+user(5)
            ELSE
...
#  endif
        ...
        END DO
      END DO
# elif defined COD_EX5
      DO k=1,N(ng)
        DO j=JstrR,JendR
          DO i=IstrR,IendR
! gentle temp gradient just to visualize the flow
            t(i,j,k,1,itemp)=T0(ng)+xr(i,j)/150000.0_r8*1.0_r8
! salty everywhere
            t(i,j,k,1,isalt)=S0(ng)
          END DO
        END DO
